...
;;;test example using time for a quick and dirty profiling to narrow where program is actually spending its time.
...
(defun bar ()
  (let ((x 5))
    (let ((result (* 2 x)))
      (format t "Result of bar: ~a~%" result)
      result)))

(defun baz ()
  (let ((x 5))
    (let ((result (* x x)))
      (format t "Result of baz: ~a~%" result)
      result)))
